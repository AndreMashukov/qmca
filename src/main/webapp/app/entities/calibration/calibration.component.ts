...
export class CalibrationComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.calibrationService
      ...
      .subscribe(
        (res: HttpResponse<ICalibration[]>) => this.paginateCalibrations(res.body, res.headers),
  ...
  trackId(index: number, item: ICalibration) {
    return item.id;
  }
  ...
  protected paginateCalibrations(data: ICalibration[], headers: HttpHeaders) {
